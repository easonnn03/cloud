...
namespace cloudass.Services
{
    public class DentalService : IDentalService
    {
        ...
        {
            var result = _dentalServiceRepository.getServiceByNameAsync(name);
            if (result != null)
            {
                var sname = result.ServiceId.ToString();
                Console.WriteLine("Service Found: " + sname);
                return result;
            }
            else
            {
                Console.WriteLine("Service not found: " + name);
                return null;
            }
        }
